...
<div class="container">
  <div class="content home" distance="55" v-pull-to-refresh="refresh">
    <v-layer></v-layer>
    <!-- 轮播图 -->
    <slider :banner="$root.planBanner" :vertical="false" style="z-index:9999; height:7.6rem;"></slider>

    <!-- 快捷入口 -->
    <bar class="home-bar">
      <bar-item path="/plan/rank" label="周盈利排行" img="/img/new/paihang.jpg" h=32 i=32></bar-item>
      <bar-item label="充值" img="/img/new/chongzhi.jpg" h=32 i=32 @click="recharge()"></bar-item>
      <bar-item path="/user/help" label="帮助" img="/img/new/bangzhu.jpg" h=32 i=32></bar-item>
    </bar>
    <!-- 盈利滚动展示 -->
    <card type="content" class-name="scrollPlan">
      <div class="row">
        <div class="col-10" style="line-height:2rem;">
          <img src="/img/专家方案/喇叭.png" style="height:0.6rem;margin-left:0.4rem; margin-top:0.55rem; opacity:0.5">
        </div>
        <div class="col-90">
    ...
    <div style="margin-bottom:4rem;">
      <v-card-container v-for="r in $root.rangeList | orderBy 'id' -1"
        ...
        <div v-if="length>0" style="margin-bottom:0.6rem; margin-top:0.4rem;">
          <card type="header" style="font-size:0.68rem;background-color:#ffffff;">
            <div style="width:180%;">
                <font style="font-size:0.68rem;color:#666666;">
                  {{r.range_name}}
                </font>
              </span>
                <font style="font-size:0.68rem;color:#666666; margin-left:0.4rem;"
                    ...
                    <div class="item-inner text-sml" style="font-size:0.68rem;margin-left:0.56rem;">
                      <div style="width: 22%;">
                        ...
                        <div style="margin-top:0.1rem;">
                          <!-- planConfident:-1默认为3 -->
                          <div v-if="p.planConfident===-1 || p.planConfident>0" style="width:.3rem; height:.3rem; border-radius:2rem; background-color:#3cdbbb; float:left; margin-left:0.1rem;"></div>
                          <div v-if="p.planConfident===-1 || p.planConfident>1" style="width:.3rem; height:.3rem; border-radius:2rem; background-color:#3cdbbb;float:left; margin-left:0.1rem;"></div>
                          <div v-if="p.planConfident===-1 || p.planConfident>2" style="width:.3rem; height:.3rem; border-radius:2rem; background-color:#3cdbbb;float:left; margin-left:0.1rem;"></div>
                          <div v-if="p.planConfident>3"  style="width:.3rem; height:.3rem; border-radius:2rem; background-color:#3cdbbb;float:left; margin-left:0.1rem;"></div>
                          <div v-if="p.planConfident>4"  style="width:.3rem; height:.3rem; border-radius:2rem; background-color:#3cdbbb;float:left; margin-left:0.1rem;"></div>
                        </div>
                      </div>
                      <div class="icon-golds" style="font-size:1rem;margin-left:0.5rem; width: 26%;">
                      ...
                      <div>
                        <img v-if="isValidate(p.deadline_time)" src="/img/new/gouwuche.png"
                          width="20" height="18" @click="addToCart(p.plan_id, $event)">
                        <img v-else="isValidate(p.deadline_time)" src="/img/new/gouwuche.png"
                          width="20" height="18" @click="addToCart(0, $event)">
                      </div>
                    </div>
                  </li>
                </div>
...
export default {
  ...
  methods: {
    ...
    addToCart (pid, e) {
      ...
      if (window.localStorage.getItem('usUser')) {
        // 添加至购物车
